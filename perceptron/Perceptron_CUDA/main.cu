...
int main() {
    ...
    // int numBatchesPerEpoch = 200; // Esto causaba el error y se colgaba

    DataLoader loader(dataPath, batchSize, inputDim, numClasses);

    // CARGAR 5 ARCHIVOS DE ENTRENAMIENTO
    ...
    // Calcular el número de lotes correctamente, redondeando hacia arriba
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (int b = 0; b < numBatchesPerEpoch; b++) {
            ...
            // Determinar el tamaño real de este lote
            // (Será 256, excepto el último lote que será 80)
            int currentBatchSize = min(batchSize, totalTrain - start);

            for (int i = 0; i < currentBatchSize; i++) { // Usar currentBatchSize
